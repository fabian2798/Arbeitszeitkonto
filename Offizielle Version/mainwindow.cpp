...
MainWindow::MainWindow(QWidget *parent) // Konstructor
  ...
  show_table(
      "SELECT day,date,type,office_time,flexible_time,summary FROM zeitkonto;");
  ...
  /*QString currentMonth = QString::number(QDate::currentDate().month());
      ...
      getDistribution(&valuesOfChart[0], &valuesOfChart[1], beginMonth ,
     ...
     *>(); delete deleted;
      ui->frame_stats_distribution->layout()->addWidget(ChartBuilder::createDistributionChart(valuesOfChart[0],
     valuesOfChart[1]));*/
...
Tagesdaten MainWindow::process_line(QString s, Tagesdaten *data,
  ...
  QString pattern_day(
      R"(^(\w{2}|\s+)\s+(\d\d?)\s+\d{1,4}\s+\|\s+(\d{1,2}:\d{2})?\s+-?\s{1,2}(\d{1,2}:\d{2})[+ *]{3}\s+(.*)\|(.*)\|(.*))");

  QString pattern_shorts(
      R"((\sFA\s|\sURL\s|\sGLT\s|\sF\s|\sABS\s|\sKRK\s))"); // Für Arbeitskürzel
                                                            // in anderen Zeile
  ...
  if (match2.hasMatch()) {
    ...
    if ((mittelteil1.contains("URL")) || (mittelteil1.contains("ABS")) ||
        (mittelteil1.contains("GLT")) || (mittelteil1.contains("KRK")) ||
        (mittelteil1 == "F")) {
    ...
    if (!endteil.isEmpty()) {
      ...
      data->setIn_otherline(
          false); // reset: Flag für Kürzel unterhalb der ersten Zeile
  ...
  if (match11.hasMatch()) {
    ...
    data->setIn_otherline(true); // Kürzel wurde unterhalb der ersten Zeile aber
                                 // noch im gleichen Tag gefunden
...
Tagesdaten MainWindow::end_calc(qint32 begin_inMin, qint32 end_inMin,
                                Tagesdaten *data) {
...
Tagesdaten MainWindow::end_flexcalc(qint32 begin_inMin, qint32 end_inMin,
                                    Tagesdaten *data) {
...
void MainWindow::toMinutesandHours(monat *m_data) {
  ...
  QString ges_zeit =
      Minutes_toString(ges_stunden) + "." + Minutes_toString(ges_minutes);
  QString ges_nt = Minutes_toString(stunden) + "." + Minutes_toString(minutes);
  QString ges_fnt =
      Minutes_toString(flstunden) + "." + Minutes_toString(flminutes);
...
void MainWindow::dateString(Tagesdaten *data, monat *m_data) {
  ...
  QString date =
      m_data->getYear() + "-" + data->getMonthInt() + "-" + data->getTages_nr();
  data->setDate(date);
}
...
// IDEE: Falls zwei verschiedene Journale des gleichen Monats geladen werden,
// werden alle Tage des Monats geupdatet
void MainWindow::update_day(Tagesdaten *data) {
  ...
  query.prepare(QString("UPDATE [zeitkonto] "
                        ...
                        "flexible_time = '%2', "
                        "summary = '%3' "
                        "WHERE strftime('%Y-%m-%d',date) IN('%4');")
                    .arg(data->getOffice_time(), data->getFlexible_time(),
                         data->getGes_Tageszeit(), data->getDate()));
  if (!query.exec()) {
...
void MainWindow::insert_table(Tagesdaten *data) {
  // TO DO: Kürzel soll angepasst werden ("" = Office, "FA" = Flexible, "GLT" =
  // Gleittag, "KRK" = Krank, "ABS" = Berufsschule, "F" = Feiertag) Kann auch
  // als Legende eingefügt werden oder durch Hover-Beschreibung
  QSqlQuery query;
  query.prepare("INSERT INTO [zeitkonto](day, date, type, office_time, "
                "flexible_time, summary) VALUES(:day, :date, :type, "
                ":office_time, :flexible_time, :summary);");
  ...
  query.bindValue(":summary",
                  data->getGes_Tageszeit()); // Netto Arbeitszeit am Tag
  db.commit();
  if (!query.exec()) {
...
void MainWindow::show_table(QString queryString) {
  ...
  ui->datalist->setHorizontalHeaderLabels(
      {"Day", "Date", "Type", "Büro", "Home", "Sum"}); // coloum names
...
void MainWindow::on_loadFile_clicked() {
  ...
  // File einlesen
  if (ui->loadfolder->isChecked() == false) {
    fileName = QFileDialog::getOpenFileName(
        this, tr("Öffne Zeus Textdatei"), "/home",
        tr("Zeus Datei (*.csv *.txt)")); // "/home" -> Startverzeichnis ändern
    qDebug() << "filename" << fileName;
    if (fileName.isEmpty()) {
      return;
    }
    workThrough(&day_data, &monats_data);
  }
  // Ordner einlesen
  else {
    QDir dir =
        QFileDialog::getExistingDirectory(this, tr("Öffne Ordner"), "/home");
    foreach (QString x, dir.entryList(QDir::Files)) {
      fileName = dir.absoluteFilePath(x);
      workThrough(&day_data, &monats_data);
    }
  }
  ...
  // Show DB
  // qDebug() << "Order: ID, Day, Date, Kürzel, Office_time, Flexible_time,
  // summary";
  show_table(
      "SELECT day,date,type,office_time,flexible_time,summary FROM zeitkonto;");
  ...
  createLogFile(monats_data.getMonth(), monats_data.getYear());

  // reset status log box
  ui->logdata->setChecked(false);
  // reset loadfolder
  ui->loadfolder->setChecked(false);
}
...
void MainWindow::fillComboBoxesFromDB() {
  ...
  if (QDate::currentDate().month() - 1 < 2) {
    ui->comboBox_begin_month->setCurrentIndex(12 -
                                              QDate::currentDate().month() - 1);
  } else {
    ui->comboBox_begin_month->setCurrentIndex(QDate::currentDate().month() - 3);
  }
  ui->comboBox_begin_year->setCurrentIndex(ui->comboBox_begin_year->count() -
                                           1);
...
void MainWindow::getDistribution(int *officesum, int *homeofficesum, QString bm,
                                 QString by, QString em, QString ey) {
  ...
  if (by == ey) {
    ...
    if (bm == em) {
      QString selectString = "SELECT date, office_time, flexible_time from "
                             "zeitkonto WHERE strftime('%m', date) = '" +
                             bm_with0 + "' ";
    ...
    // für mehrere Monate
      ...
      QString selectString =
          "SELECT date, office_time, flexible_time from zeitkonto ";

      for (auto x : *months) {
        if (x == months->last()) {
          selectString += "strftime('%m', date) ='" + x +
                          "') AND (strftime('%Y', date) = '" + by + "')";
  ...
  // Jahresübergreifende Eingabe
    ...
    QString query = "SELECT date, office_time, flexible_time from zeitkonto "
    ...
    // Bei allen anderen Jahren ( z.B. Eingabe zwischen 2020 und 2024, dann
    // werden hier die Jahre 2021 bis 2023 behandelt
    if (ey.toInt() - 1 != by.toInt()) {
      int i;
      for (i = by.toInt() + 1; i < ey.toInt(); i++) {
        p->addItem("über " + QString::number(i));
        query = "SELECT date, office_time, flexible_time from zeitkonto WHERE "
                "strftime('%Y', date) = '";
        ...
        while (middleYear.next()) {
          ...
          // qDebug() << firstYear.value(0).toString() << " " <<
          // firstYear.value(1).toString() << " " <<
          // firstYear.value(2).toString();
        }
      }
    }
...
void MainWindow::on_pushButton_stats_update_clicked() {
  ...
  QString begin_month =
      QString::number(ui->comboBox_begin_month->currentIndex() + 1);
  QString end_month =
      QString::number(ui->comboBox_end_month->currentIndex() + 1);
  QString begin_year = ui->comboBox_begin_year->currentText();
  QString end_year = ui->comboBox_end_year->currentText();

  getDistribution(&valuesOfChart[0], &valuesOfChart[1], begin_month, begin_year,
                  end_month, end_year);
  ...
  ui->frame_stats_distribution->layout()->addWidget(
      ChartBuilder::createDistributionChart(valuesOfChart[0],
                                            valuesOfChart[1]));
}
...
void MainWindow::on_pushButton_filter_table_clicked() {
  ...
  filterString += "WHERE ( date BETWEEN '" +
                  ui->dateEdit_begin_date->date().toString("yyyy-MM-dd") +
                  "' AND '";
  ...
  // Checkboxen werden überprüft, gegebenenfalls werden die Bedingungen
  // hinzugefügt.
...
void MainWindow::loggingDays(Tagesdaten *day, QString year) {
    ...
    QString log =
        QString("%1| %2 | %3 | %4 | %5\n")
            .arg(QDateTime::currentDateTime().toString("dd-MM-yyyy hh:mm:ss"),
                 year + "-" + day->getMonthInt() + "-" + day->getTages_nr(),
                 day->getTag(), day->getArb_art(), day->getGes_Tageszeit());
    Logs.append(log);
  }
}

void MainWindow::createLogFile(QString month, QString year) {
  if (ui->logdata->isChecked()) {
    QString logfilename = QFileDialog::getSaveFileName(
        this, "Datei speichern",
        "/" + QDate::currentDate().toString("dd_MM_yyyy") + "_LogFile",
        "(*.log)");
...
void MainWindow::workThrough(Tagesdaten *day_data, monat *monats_data) {
  qint32 flexgeht_int;
    ...
    *day_data = process_line(line, day_data, monats_data);
    ...
    if (day_data->getEnd_line() == true) {
      ...
      if (day_data->getKaug() == false) { // Doppelbuchung verhindern
        day_data->setGesamte_tageszeit(
            day_data->getFlexNetto_int(), day_data->getNetto_int(),
            day_data->getPausenzeit()); // (Flex+NT) - Pause
      }
      qDebug() << "Pausenzeit: " << day_data->getPausenzeit();

      // Soll doppelte Buchung der Pausenzeit verhindern
      // Monatsdaten werden jeden Tag um die rohe Arbeitszeit(mit Pausenabzug)
      // addiert
      if (day_data->getNetto_int() > 0 && day_data->getFlexNetto_int() == 0) {
        monats_data->setGes_Nettozeit(day_data->getNetto_int() -
                                      day_data->getPausenzeit());
        day_data->setOffice_time_pause(); // officetime = netto - pause,
                                          // flexible_time = flex
      }
      if (day_data->getFlexNetto_int() > 0 && day_data->getNetto_int() == 0) {
        monats_data->setGes_Flexnettozeit(day_data->getFlexNetto_int() -
                                          day_data->getPausenzeit());
        day_data->setFlexible_time_pause(); // flexible_time = flex - pause,
                                            // office_time = netto
      }
      // Soll Minuszeiten verhindern falls Arbeitszeit zu kurz zum Pausenabzug
      // war
      if (day_data->getNetto_int() > 0 && day_data->getFlexNetto_int() > 0) {
      ...
      qDebug() << "Tagesarbeitszeit als Timestring: "
               << day_data->getGes_Tageszeit();
      ...
      // Logging
      loggingDays(day_data, monats_data->getYear());
